...
satisfiable expr = do
  Z3.push
  Z3.assert expr
  --tmp <- Z3.solverGetHelp
  --validated <- trace tmp Z3.solverCheck
...
squeezeLoopInfo :: (Vars, Bool) -> [ProcDecl] -> Int -> Bound -> Ident -> Moderator -> Expr -> [Stmt] -> Bool -> Z3 Vars
squeezeLoopInfo (scope, invalid) ast state bound name m cond body forward =
  processExpr scope cond >>= \case
    Right z3cond -> do
      ...
      tmpScope <-
        if invalid
          then return scope
          else invalidateVars scope $ (getVarName (getModVar m)) : modifiedVars ast body
  ...
  where
    ...
    processForLoop scope forward inv var@(Var t name (Just val) p1) body m@(Moderator v op incr) cond
     pos warnings =
                  ...
                  if not doOpt
                      ...
                      (Just (Invariant inv' _), True) -> do
                        ...
                        if bound' <= unrollBound
                          then do
                            ...
                            (scope'', _, _, _) <-
                              invariant inv' body name scope' warnings
                    ...
                    -- Do optimization run
                      ...
                      (Just (Invariant inv' _), True) -> do
                        ...
                        (_, inf', _, warnings') <- invariant inv' body name scope' warnings
                        ...
                        if incr' > 0
                          then do let bound' =
                            ...
                            (tmpScope, body', validated, warnings'') <-
                              generalizedAnalysis (scope', False) body z3expr m cond state warnings'
                      ...
                      (Just (Invariant inv' _), False) -> do
                        ...
                        (_, body'', validated, warnings'') <-
                          generalizedAnalysis (scope', True) body z3expr m cond state warnings'
                        Z3.pop 1
      ...
      where
        generalizedAnalysis ::  (Vars, Bool) -> [Stmt] -> AST -> Moderator -> Expr -> Int -> [String] -> Z3 (Vars, [Stmt], Bool, [String])
        generalizedAnalysis (scope, invalid) body z3expr m cond state warnings = do
          -- Trying to get information for the generalized analysis
          -- To convey whether the loop variable may overflow!
          tmpScope <- squeezeLoopInfo (scope, invalid) ast state bound name m cond body forward
        ...
        -}
        invariant :: Expr -> [Stmt] -> Ident -> Vars -> [String] ->
          Z3 (Vars, LoopInfo, [Stmt], [String])
        invariant inv body loopVar loopScope warnings'  =
            ...
            Right z3inv -> do
              ...
              case validated of
                Right True -> do
                  ...
                  Z3.push
                  loopScope' <- invalidateVars loopScope $ (getVarName (getModVar m)) : modifiedVars ast body
                  -- Z3.assert z3inv
                  Z3.assert =<< prepareInfo loopScope' inv

                  -- Need to assert that the end condition is not true, as maintenance is start of
                  -- each iteration
                  Z3.assert =<< Z3.mkNot =<< Z3.mkEq (fst (fromJust (tryGetVar loopVar loopScope'))) =<< prepareInfo loopScope' cond

                  -- Arbitrary iteration
                  (_, scope, _, _, _) <- loopBody ast False forward loopScope' body m state bound warnings

                  -- validated <- validate stmt z3inv
                  validated <- validate stmt =<< prepareInfo scope inv

                  ----------------------------
                  -- (if doOpt
                  --   then do
                  --     trace ("\nTesting invariant ("++(show forward)++ ") " ++ show inv ++ "\n") $return()
          ...
          where
            prepareInfo :: Vars -> Expr -> Z3 AST
            prepareInfo scope inv =
              processExpr scope inv >>= \case
                Right z3Inv -> return z3Inv
                _ -> error "bam"
              ...
              if bound <= unrollBound
                then do
                  ...
                  (scope', body', validated, warnings') <-
                    generalizedAnalysis (scope, False) body val m cond state warnings
...
processProgram (Program decls) doMain = do
  ...
  ps <- Z3.mkParams
  p <- Z3.mkStringSymbol "threads"
  Z3.paramsSetUInt ps p 8
  p2<- Z3.mkStringSymbol "threads.cube_frequency"
  Z3.paramsSetUInt ps p2 16
  Z3.solverSetParams ps
